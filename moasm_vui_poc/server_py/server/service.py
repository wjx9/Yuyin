...
from .navigation_integration import try_handle_navigation
...
class ChatService:
    ...
    def handle_chat(self, req: ChatRequest) -> ChatResponse:
        ...
        # 导航引擎前置拦截：导航相关请求由导航对话引擎处理，
        # 避免"陷入"高德子Agent，保证非导航意图由主助手处理。
        nav_resp = try_handle_navigation(req)
        if nav_resp is not None:
            _log.info("导航引擎已处理: session=%s intent=%s", req.session_id, nav_resp.intent)
            return nav_resp
