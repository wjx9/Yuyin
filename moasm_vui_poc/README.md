...
- **凭证自动复用**（2026-08 新增）：启用同 MCP server 的多个技能时，第一个填了 Token，后续技能自动复用，无需重复填写（如麦当劳 5 个技能共用一个 Token）。
...
Flutter 客户端集成高德导航 SDK，导航 UI 以**悬浮面板**形式叠加在聊天界面之上，导航期间语音助手始终可见可用。

- **实现方式**：Flutter PlatformView（Hybrid Composition）嵌入原生 `AMapNaviView`，封装为 `NaviFloatingPlatformView`，管理完整生命周期（算路→`startNavi(GPS)`→事件推送→`dispose` 停止导航）。
- **触发链路**：用户语音"导航到 X"→ 云侧返回 `navCommand(cmd=4)` → Flutter 设置 `pendingNavigation` → ChatPage 显示悬浮面板 → PlatformView 自行完成导航。
- **语音协同**：助手听写/播报时通过 `setUseInnerVoice(false)` 静音导航播报，空闲恢复；导航实时数据（转向/距离/时间）通过 EventChannel 推回，供眼镜 HUD 展示。
- **键盘兼容**：`windowSoftInputMode="adjustPan"` 避免输入法弹出时悬浮窗 SurfaceView 空白。
- 技术设计详见 [`高德导航接入技术设计文档.md`](高德导航接入技术设计文档.md)。
