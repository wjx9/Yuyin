...
本目录是这套**多能力分流语音助手的手机端**：把 `chat_app.py` 的体验从"PC 终端打字"搬到"手机语音"，
业务"大脑"仍在服务端（`serve.py`），客户端只负责**采集语音 → 发请求 → 渲染/朗读回复**。
